...
def update_edge_types(
    ...
    try:
        ...
        for row in changed_data:
            if "ID" in row:
                ...
                if result.get("success"):
                    ...
                    error_msg = result.get("message", "Unknown error")
                    errors.append(
                        f"Failed to update edge type {edge_type_id}: {error_msg}"
                    )
        ...
        if errors:
            message = (
                f"Updated {updated_count} edge types. Errors: {'; '.join(errors[:2])}"
            )
            if len(errors) > 2:
            ...
            return (
                True,
                f"Successfully saved changes to {updated_count} edge type(s)",
                updated_data,
            )
    ...
    try:
        ...
        for row in selected_rows:
            ...
            if result["success"]:
                ...
                errors.append(
                    f"Failed to delete Edge Type {row['Name']}: {result.get('message', 'Unknown error')}"
                )
        ...
        if errors:
            message = (
                f"Deleted {deleted_count} edge types. Errors: {'; '.join(errors[:3])}"
            )
            if len(errors) > 3:
            ...
            return (
                True,
                f"Successfully deleted {deleted_count} edge type(s)",
                updated_data,
            )
...
def toggle_delete_modal(
    delete_clicks, confirm_clicks, cancel_clicks, is_open, selected_rows, data
):
