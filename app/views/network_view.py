...
#                             'borderColor': '#0b5ed7',      // Darker blue for border
#                             'borderWidth': '4px'
#                         }
#                     },
#                     {
#                         selector: '.connected',
#                         style: {
#                             'backgroundColor': '#b3d9ff',  // Lighter blue for connected nodes
#                             'borderColor': '#0d6efd',      // Bootstrap primary blue
#                             'borderWidth': '3px'
#                         }
#                     },
...
class NetworkView:
    ...
    def get_cytoscape_client_callback() -> str:
        """
        Load the JavaScript code for Cytoscape client-side callback from external file.
        Falls back to inline version if file not found.
        """
        try:
            # Try to load from external JS file
            js_path = Path(__file__).parent.parent / 'assets' / 'js' / 'cytoscape_callback.js'
            
            if js_path.exists():
                with open(js_path, 'r', encoding='utf-8') as f:
                    js_content = f.read()
                
                # Wrap the external function to match Dash clientside callback signature
                return f"""
                {js_content}
                
                // Return the wrapped function for Dash clientside callback
                window.cytoscapeCallback
                """
            else:
                print(f"Warning: JavaScript file not found at {js_path}, using inline fallback")
                return NetworkView._get_inline_callback()
                
        except Exception as e:
            print(f"Error loading external JavaScript: {e}, using inline fallback")
            return NetworkView._get_inline_callback()
    
    @staticmethod
    def _get_inline_callback() -> str:
        """Fallback inline JavaScript callback (simplified version)"""
        return """
        function(networkDataJson, filteredValue) {
            console.log('Using inline fallback callback');
            // Add minimal inline version here if needed
            return '';
        }
        """